...
| `nonconformantItems` | `object[]` | 아니오 | `[]` | (모드 B) 호출자(full-pipeline 10.5단계)가 `ConformanceResult.scenarioResults[]`에서 id가 `unmet`(string[] ID 배열)에 포함되고 `verdict:"unsatisfied"`인 항목을 **조인해 구성** — `{scenarioId, testClass, testMethods, verdict, nonconformanceClass, notes}` (`unmet` 자체에는 이 필드들이 없다) |
