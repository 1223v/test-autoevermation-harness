...
```
[Test-AutoEverMation#0.27.0]                                   ← 파이프라인 없음(버전만)
[Test-AutoEverMation#0.27.0] <progress>% | stage 4: generate-scenarios ← full-pipeline 진행 중
[Test-AutoEverMation#0.27.0] <progress>% | ↩ resumed @ stage 8: measure-coverage ← 상태 복원 재개(durable resume)
[Test-AutoEverMation#0.27.0] 100% | done (ok)                  ← 유효한 schema v2 완료 결과
```
