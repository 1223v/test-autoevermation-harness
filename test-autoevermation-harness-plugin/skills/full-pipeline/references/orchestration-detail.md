...
| 3~9단계 | 서브에이전트 파이프라인 (순차, 9는 opt-in) | 이전 단계 산출물에 강하게 의존 |
| 보정·커버리지·활성화된 뮤테이션 루프 | 생성-검증(producer-reviewer) | 무진전 감지로 무한루프 방지 |
...
| `pitestReport.present` | `09_mutation_result.json` | 활성화된 실행의 실제 `mutationScore` + `thresholdMet` 판정. 비활성화 시 리포트 없이 `PITEST_DISABLED` skip 산출물을 새로 기록 |
...
**게이트 훅 호환**: `guard-gate-artifacts.py`는 `08/09` 산출물이 `gatePassed/thresholdMet:false`인데 loop 증거(`iterations≥1`·`remainingGaps`/`survivingMutants`)가 없으면 Write를 막는다. 유일한 예외는 현재 설정의 `mutation.enabled:false`에서 새로 기록하는 `{status:"skipped",reason:"PITEST_DISABLED",mutationScore:null,thresholdMet:null,iterations:0}`이다. 활성화된 복원 stub은 **실제 리포트 파싱값**을 담아 불변식을 충족시킨다. 그 밖에 `status` 필드로 우회하지 말 것.
...
|---|---|---|
| "이 패키지/클래스만 다시" | 2(ast)→3→3.5(권고 게이트)→4→4.5→5→6→8→(enabled일 때 9)→10 (대상 스코프 한정) | 01_spec |
| "스펙 문서 추가했어" | 1(spec)→4→4.5→5→6→8→(enabled일 때 9)→10 | 02_ast, 03_source, 03b·03c(권고 필터) |
| "리팩토링 권고만 다시" | 3.5(판정+게이트)→4→4.5→5→6→8→(enabled일 때 9)→10 | 01~03 |
| "시나리오 다시/승인 다시" | 4(scenario)→4.5(승인)→5→6→8→(enabled일 때 9)→10 | 01~03, 03b·03c |
| "커버리지만 더 올려" | 8(measure-coverage 루프)→10 | 01~06, 04b |
| "뮤테이션만 다시" | 요청 자체를 opt-in으로 보고 `mutation.enabled:true` 확정 → 9(mutation-test 루프)→10 | 01~08, 04b |
...
| "시나리오 적합성 불일치 고쳐" | 10.5(unsatisfied→fixer 모드 B / missing→부분 재생성)→6→10 (테스트 변경 시 8 및 enabled일 때 9 회귀) | 01~09, 04b, 10_conformance |
| **`_workspace/` 없음 + 하네스 생성 테스트 존재**(`harnessProvenance:true`, "보완/수정/업데이트") | **6(run)→8→(enabled일 때 9)→10** (CI 기본; 대화형의 9 선택지도 enabled일 때만 표시) | detect_pipeline_state로 복원한 stub 01~05·04b (§2-1) |
...
| 커버리지/활성화된 뮤테이션 미수렴 | 진전이 있으면 계속 재시도; **무진전 3회 연속**이면 `partial` + 잔여 gap/survivor 전량 보고(임의 제외/무시 금지) |
...
| 무효 게이트 산출물 (#21) | `08`의 기존 무효 조건, 활성화된 `09`의 기존 무효 조건, 또는 정확한 `PITEST_DISABLED` 계약이 아닌 `skipped`는 무효. 비활성화 정상 skip만 예외. `guard-gate-artifacts.py` 훅이 무효 Write를 차단 |
