...
|---|---|---|
| `hasTests` / `testFiles[]` | `05_test-gen_files.json` | `{status:"reused", files:[{path}], source:"durable-scan"}` |
...
| **`_workspace/` 없음 + 테스트 존재("보완/수정/업데이트")** | **6(run)→8→9→10** (CI 기본; 대화형은 `AskUserQuestion`으로 6/8/9/4 선택) | detect_pipeline_state로 복원한 stub 01~05·04b (§2-1) |
