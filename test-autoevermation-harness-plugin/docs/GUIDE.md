...
필요한 것은 PATH의 **Python 3.10+** 뿐이다. 자동 설치가 실패한 환경(오프라인 등)의 수동 폴백:
...
| 증상 | 원인 | 해결 |
|---|---|---|
| MCP 서버 3종이 연결 안 됨 | (v0.12.0+ 자동 설치 실패 — 오프라인/`python3 -m venv` 불가/Python<3.10) | `mcp-logs-plugin-*` 로그의 bootstrap 진단 확인. 수동 폴백: `python3 -m pip install -r mcp/requirements.txt` 후 `/reload-plugins` |
