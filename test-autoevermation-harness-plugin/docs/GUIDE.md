...
**Python 자체도 없어도 된다**(v0.13.0+, macOS/Linux) — PATH에 3.10+가 없으면 `mcp/run-server.sh`가
uv(무-sudo, `~/.local`)로 관리형 Python을 자동 설치한다(`HARNESS_AUTO_PYTHON=0`으로 비활성화,
Windows 네이티브는 수동 설치 필요). 자동 설치가 실패한 환경(오프라인 등)의 수동 폴백:
...
| 증상 | 원인 | 해결 |
|---|---|---|
| MCP 서버 3종이 연결 안 됨 | (자동 설치 실패 — 오프라인/curl·wget 없음/Windows 네이티브/`HARNESS_AUTO_PYTHON=0`) | `mcp-logs-plugin-*` 로그의 `run-server`/`bootstrap` 진단 확인. 수동 폴백: Python 3.10+ 설치 → `python3 -m pip install -r mcp/requirements.txt` → `/reload-plugins` |
