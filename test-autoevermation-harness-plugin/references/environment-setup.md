...
node "<pluginRoot>/mcp/launch.cjs" --ensure-only        # E1(uv 관리형 Python, 무-sudo) + E2(${CLAUDE_PLUGIN_DATA}/venv에 mcp[cli]>=1.2.0)
#   (POSIX 전용 구진입점: sh "<pluginRoot>/mcp/run-server.sh" --ensure-only — 수동 폴백으로 유지)
