...
|---|---|---|---|---|---|---|
| E1 | **Python 3.10+** | `python3 -c "import sys;assert sys.version_info>=(3,10)"` 또는 `${CLAUDE_PLUGIN_DATA}/python-path` 핀 존재 | auto(macOS/Linux) | **자동**(v0.13.0+): `sh <pluginRoot>/mcp/run-server.sh --ensure-only` — PATH에 3.10+ 없으면 uv(무-sudo, `~/.local`)로 관리형 Python 자동 설치. 실패 시에만 설치 경로 안내(brew/apt/python.org). `HARNESS_AUTO_PYTHON=0`이면 assist로 강등 | 동일 자동, 실패 시 하드 중단 + remediation | MCP 런타임 |
...
```bash
# E1+E2: Python + MCP Python SDK — v0.13.0+ 자동(권장): Python이 없으면 uv로 설치 후 venv 준비
sh "<pluginRoot>/mcp/run-server.sh" --ensure-only       # E1(uv 관리형 Python, 무-sudo) + E2(${CLAUDE_PLUGIN_DATA}/venv에 mcp[cli]>=1.2.0)
