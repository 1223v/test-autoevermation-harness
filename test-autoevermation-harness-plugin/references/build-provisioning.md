...
이 문서는 **대상 프로젝트의 빌드 파일**이 필수 커버리지 단계를 돌릴 수 있도록 준비되어 있는지 점검하고(가능하면 함께 세팅), **콜드 의존성 캐시**로 인한 첫 실행 실패를 막는 **단일 출처(SSOT)**다.
...
**대상 프로젝트의 빌드 파일**이 JaCoCo XML을 낼 수 있는지는 별개이며, 준비돼 있지 않으면 8단계(measure-coverage)가 깨진다.
...
**감지(신호 전용).** `build-test.detect_build_capabilities(root)`는 빌드 파일을 읽어 `capabilities{jacoco, jacocoXml}` + `missing[]` + 각 필수 누락에 대한 `proposedChanges[{file, anchor, snippet, reason, source}]`(최소 스니펫)을 반환한다. **파일을 수정하지 않는다.**

**처리(detect → approve → inject).**

- **대화형**: JaCoCo 누락과 최소 스니펫을 보여준다. 승인한 `proposedChanges`만 빌드 파일에 최소 주입하고 `PipelineResult.buildChanges[]`에 기록한 뒤 재감지한다. **침묵 주입 금지.**
- **비대화형·CI**: JaCoCo 또는 XML 출력이 누락된 경우 자동 주입하지 않고 `status:"failed"` + remediation으로 중단한다.
