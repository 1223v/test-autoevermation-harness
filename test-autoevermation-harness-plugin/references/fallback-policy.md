...
| 21 | **커버리지 게이트 산출물 유효성 + advisory 비면제**(8단계) | RA advisory는 8단계 게이트의 면제 사유가 아니다. **무효 조건**: `08_coverage_result.json`이 `gatePassed:false` ∧ (`iterations<1` ∨ `remainingGaps` 빈 배열)이면 실제 보완 루프와 잔여 전량 보고가 없으므로 무효다. `guard-gate-artifacts.py`가 이를 차단한다 | `full-pipeline` 8단계; `measure-coverage`; `guard-gate-artifacts.py` |
