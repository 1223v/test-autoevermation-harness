...
| 7 | **활성화된 PITest 버전 폴백/graceful skip** | `mutation.enabled:false`이면 분기 자체가 없음. 활성화 후 호환 불가 시 대화형=AskUserQuestion으로 "호환 버전 폴백 / PITest 비활성화 후 skip / 중단" 선택, CI=명시 opt-in 실패이므로 중단 | `mutation-test`, `mutation-analyst` |
...
- repair-tests → run-tests, coverage-closer → measure-coverage, 그리고 활성화된 경우의 mutation 강화 루프를 **게이트 충족까지 반복**한다.
