...
| 21 | **커버리지 게이트 산출물 유효성 + advisory 비면제**(8단계) | RA advisory는 8단계 게이트의 면제 사유가 아니다. **무효 조건**: `08_coverage_result.json`이 `gatePassed:false` ∧ (`iterations<1` ∨ `remainingGaps` 빈 배열)이면 실제 보완 루프와 잔여 전량 보고가 없으므로 무효다. **v0.31.0: 이를 차단하는 훅은 없다** — 무효 산출물을 걸러내는 책임은 8단계 스킬 자신에게 있다 | `full-pipeline` 8단계; `measure-coverage` |
