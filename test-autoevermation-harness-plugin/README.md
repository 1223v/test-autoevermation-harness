...
MCP 연결은 `.mcp.json`(Python으로 연결), 선택적 LSP(JDT LS)는 `plugin.json`의 `lspServers`로 `.lsp.json`을 등록(`jdtls` 미설치 시 graceful — `/plugin` 에러 탭 표시만, AST-only degrade).
...
| JDT LS (선택) | 최신 | Java 21+ runtime | `plugin.json` `lspServers`→`.lsp.json` 등록(미설치 시 graceful) |
