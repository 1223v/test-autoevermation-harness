...
| **JDK 21+ (필수)** | `java -version` | JavaParser jar 빌드(17+)와 JDT LS 구동(21+)을 아우르는 단일 기준. 미탐지 시 `setup-harness`(E4/E7)가 remediation과 함께 하드 중단한다. 상세: [DEPENDENCIES.md](./DEPENDENCIES.md) |
...
> 빌드/런타임 산출물이므로 복사하지 않아도 되고, JavaParser jar는 `setup-harness`(E6)가 동봉된 `mvnw`로
> `./mvnw -q -DskipTests package` 자동 빌드한다(v0.16.0부터 필수 — 실패 시 `JAVAPARSER_REQUIRED`로 하드 중단).

### 3) 환경 세팅 (최초 1회 — 필수)

설치 후 세션에서 **한 번** 아래를 실행한다. 환경 세팅(Python·MCP 런타임·MCP 라이브 연결·JDK·JavaParser jar·JDT LS·상태줄)을 전부 끝내 준다.

```text
/test-autoevermation-harness-plugin:setup-harness
```

- **v0.24.0부터 `full-pipeline`은 환경을 세팅하지 않는다** — 시작 시 검증(E-verify)만 하고, 미완료면
  `"먼저 /test-autoevermation-harness-plugin:setup-harness 를 실행해 환경 세팅을 완료하세요"`와 함께 **하드 중단**한다.
- 멱등하다. 환경이 깨졌을 때(플러그인 업데이트로 venv 경로 변경, `mvn clean`으로 jar 소실, JDK 교체 등)
  그대로 다시 실행하면 **빠진 항목만** 복구한다.
- CI(`claude -p`)에서는 질문 없이 결정적 항목(런타임·jar·JDT LS)을 자동 세팅하고, 미충족 시 하드 중단한다.
...
- **`setup-harness`의 `E3b`가 `health` 도구 3종을 실제로 호출해 연결을 세팅·검증하고, 파이프라인 시작 전
  E-verify 프로브가 이를 재확인한다**(v0.16.0+, v0.24.0 분리) — 실패 시 대화형/CI 모두 remediation과 함께
  하드 중단하며, silent degrade로 계속 진행하지 않는다.
...
9. 선택적 뮤테이션 강화 (`mutation-test`, `mutation.enabled:true`일 때)
...
> **흐름도**: 전체 구동 흐름(setup-harness 세팅·E-verify 게이트·승인 게이트·적합성 검증·fallback 분기)을 Mermaid로 시각화한 문서는
...
③ 선행 산출물 없는 후속 기록(순서 게이트), ④ 8단계와 활성화된 9단계의 무효·위조 산출물을 deny하되 정확한 `PITEST_DISABLED` skip은 허용한다.
...
| Skill | 호출 방법 | 역할 |
|---|---|---|
| **`setup-harness`** | `/test-autoevermation-harness-plugin:setup-harness` | **환경 세팅 전용(선행 필수)** — E1~E10(Python·MCP 런타임·MCP 라이브 연결·JDK 21+·JavaParser jar·JDT LS) + 상태줄 설치. 멱등 |
| `configure-harness` | `/test-autoevermation-harness-plugin:configure-harness` | 세팅 검증 게이트(E-verify) + 인터랙티브 설정 → `HarnessConfig` 생성 (**환경을 세팅하지 않는다**) |
...
| `mutation-test` | `/test-autoevermation-harness-plugin:mutation-test` | 선택적 PITest 뮤테이션 강화 루프(수동 호출은 opt-in) |
...
MCP 연결은 `.mcp.json`(Python으로 연결)이며, `setup-harness`의 `E3b`가 `health` 3종을 실호출해 세팅·검증하고, 파이프라인 시작 전 E-verify 프로브가 재확인해
...
- **환경 세팅 선행(`setup-harness`, v0.24.0에서 전용 명령으로 분리)**: [environment-setup.md](references/environment-setup.md)
  체크리스트(E1~E10 — MCP SDK·JavaParser jar·JDT LS·JDK·테스트 실행 JDK)와 상태줄을 TODO로 **선세팅**한다 —
  대화형=항목별 함께 세팅, CI=결정적 항목 자동 세팅. `full-pipeline`/`configure-harness`는 세팅하지 않고
  **E-verify 프로브로 검증만** 하며, 미충족이면 파이프라인을 시작하지 않고 `setup-harness` 실행을 안내한다.
  (빌드도구·Spring 프로파일은 0.5단계, 대상 빌드 능력·캐시는 0.6단계 소관.)
- **인터랙티브 설정(`configure-harness`)**: 파이프라인 시작 시 AskUserQuestion으로 4항목을 질문해
  도메인 특화 `HarnessConfig`를 구성한다 — ① 스펙 문서 경로 추가, ② 테스트 대상 폴더/패키지 선별,
  ③ PITest 사용 여부(`mutation.enabled`, 기본 `false`)와 활성화 시 깊이·대상, ④ 커버리지 임계값·제외 규칙. 비대화형 CI는 명시적 `true`만 PITest opt-in으로 인정한다.
...
- **선택적 뮤테이션 강화(`mutation-test` + `mutation-analyst`)**: 기본은 `mutation.enabled:false`라 9단계를 정상 `PITEST_DISABLED`로 건너뛴다. 활성화하면 PITest로 살아남은 mutant를 찾아 단언을 강화(기본 mutation score ≥ 0.80). 활성화 후 임의 스킵과 sleep/over-mock/broad-catch는 금지한다.
...
`setup-harness`가 **하네스 런타임**만 세팅하던 공백을 메운다. 대부분의 실제 프로젝트는 JaCoCo **XML**(Gradle 기본 OFF)이 없어 커버리지(8단계)가 깨질 수 있다. PITest는 선택 기능이지만 활성화했다면 plugin/JUnit 어댑터/**XML 출력**이 모두 필요하다. 네트워크 기본 OFF(`--offline`)의 콜드 캐시 문제까지 **0.6단계**에서 선제 처리한다(정본: [references/build-provisioning.md](./references/build-provisioning.md)).

- **빌드 능력(#17)**: `detect_build_capabilities(..., require_pitest=mutation.enabled)`가 JaCoCo XML을 항상, PITest plugin/JUnit/XML은 opt-in 시에만 필수 판정한다. 대화형은 승인 후 최소 스니펫 주입, CI는 JaCoCo 또는 명시 활성화된 PITest 누락만 remediation 중단한다.
...
| PITest (Gradle, opt-in) | gradle-pitest 1.19.0 | — | `junit5PluginVersion 1.0.0`, `outputFormats`에 XML 포함 |
...
| `examples/gradle/build-boot2.gradle` | (Boot 2.x) Groovy DSL, Java 8/11, JaCoCo + 선택적 PITest/JUnit4 폴백 주석 |
| `examples/maven/pom-snippet.xml` | (Boot 4.x) Maven Surefire/Compiler 설정 스니펫 |
| `examples/maven/pom-snippet-boot2.xml` | (Boot 2.x) Java 8 parent + JaCoCo + 선택적 PITest/JUnit4 폴백 주석 |
