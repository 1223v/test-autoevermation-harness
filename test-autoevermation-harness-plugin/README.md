...
| `build-test` | `mcp/build_test_server.py` | `detect_build_tool`, `detect_spring_profile`, `detect_build_capabilities`(v0.8), `check_dependency_cache`(v0.8), `list_test_tasks`, `run_targeted_tests`(`online=` 프라이밍), `parse_junit_xml`, `parse_jacoco_report`, `coverage_gate`, `detect_pipeline_state`(v0.20 — 영속 증거 기반 상태 복원), `health` |
...
## v0.2 신규: near-100% 커버리지 · 인터랙티브 설정
...
- **인터랙티브 설정(`configure-harness`)**: 파이프라인 시작 시 AskUserQuestion으로 3항목을 질문해
...
`setup-harness`가 **하네스 런타임**만 세팅하던 공백을 메운다. 대부분의 실제 프로젝트는 JaCoCo **XML**(Gradle 기본 OFF)이 없어 커버리지(8단계)가 깨질 수 있다. 네트워크 기본 OFF(`--offline`)의 콜드 캐시 문제까지 **0.6단계**에서 선제 처리한다(정본: [references/build-provisioning.md](./references/build-provisioning.md)).

- **빌드 능력(#17)**: `detect_build_capabilities(root)`가 JaCoCo 플러그인과 XML 출력을 필수 판정한다. 대화형은 승인 후 최소 스니펫을 주입하고, CI는 누락 시 remediation으로 중단한다.
...
- **적합성 검증(9단계)**: `verify-scenarios` + `scenario-conformance-verifier`가 `scenarioRef`(메서드명 `sc001_…` +
  ...
  `unmet`이 있으면 9.5단계 자동 보정 루프를 수행하고, 소진 후에도 남은 항목은 `status: partial`로 전량 보고한다.
- **산출물 구조**:
...
| `examples/gradle/build-boot2.gradle` | (Boot 2.x) Groovy DSL, Java 8/11, JaCoCo + JUnit4 폴백 주석 |
| `examples/maven/pom-snippet.xml` | (Boot 4.x) Maven Surefire/Compiler 설정 스니펫 |
| `examples/maven/pom-snippet-boot2.xml` | (Boot 2.x) Java 8 parent + JaCoCo + JUnit4 폴백 주석 |
