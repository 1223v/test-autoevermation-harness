...
Phase E가 **하네스 런타임**만 세팅하던 공백을 메운다. 대부분의 실제 프로젝트는 JaCoCo **XML**(Gradle 기본 OFF)이 없어 커버리지(8단계)가 깨질 수 있다. PITest는 선택 기능이지만 활성화했다면 plugin/JUnit 어댑터/**XML 출력**이 모두 필요하다. 네트워크 기본 OFF(`--offline`)의 콜드 캐시 문제까지 **0.6단계**에서 선제 처리한다(정본: [references/build-provisioning.md](./references/build-provisioning.md)).
