...
- full-pipeline **밖에서**(또는 파이프라인 종료 후) 사용자가 지목한 기존 테스트 코드를 직접 편집하는 `edit-tests` 스킬과 `test-editor` 에이전트를 신설했다. 테스트 메서드 이름 변경, 단언 추가·조정, 테스트 파일 정리·리팩토링을 대상으로 하며, 모든 편집은 `repo-ast` MCP의 AST 분석에 근거한다. 이 에이전트는 Bash가 없어 테스트를 실행하지 않고(편집 전용), 재실행은 `run-tests`로 안내한다.
- 실패·에러 테스트 보정은 `repair-tests`, 테스트 재생성·커버리지 상향은 `full-pipeline`가 계속 담당한다 — `edit-tests`는 트리거가 겹치지 않게 분리했고, 범위 밖 요청은 편집 없이 해당 스킬로 회송한다.
- `src/test/java` 기록 가드(`guard-gate-artifacts.py`의 `TEST_WRITE_AGENTS`)에 `test-editor`를 추가해, 파이프라인이 돈 것과 같은 세션에서도 사용자 지시로 기존 테스트를 편집할 수 있게 했다(4.5 승인 게이트·순서 게이트·타 에이전트 provenance는 그대로 유지). 훅·`.mcp.json`·`plugin.json` 열거는 변경하지 않는다(에이전트·스킬 자동 발견).
- 플러그인/마켓플레이스 버전을 `0.26.0`으로 올렸다. 배포 구성은 스킬 15종·에이전트 11종·MCP 서버 3종이다. MCP 패키지 버전은 변경 없다(`0.11.1`).

---

## [0.25.1] - 2026-07-15

### Fixed — 재개 증거와 산출물 가드 보강

- 심볼릭 링크를 실제 경로로 정규화해 `_workspace/` 및 테스트 소스 가드를 우회할 수 없게 했다.
- durable scan 마커를 실제 `detect_pipeline_state` 요청 root·커버리지 임계값과 응답에 연결하고, 응답이 증명한 04/05/06/08 산출물만 재사용할 수 있게 했다. `projectRoot`가 Claude Code의 cwd와 달라도 대상 프로젝트에 run/spawn/detect 마커를 라우팅하며, allowlist에 없는 `source:"durable-scan"` 산출물은 일반 산출물로 폴백하지 않는다. 08 stub 기록 시 현재 config 임계값을 다시 대조해 detect 뒤 설정이 바뀐 stale 증거도 거부한다. 마커 디렉터리는 훅만 기록할 수 있다.
- 테스트가 0개인 JUnit XML을 성공 증거로 인정하지 않으며, 이전 실행의 09 산출물이 남아 있어도 새 실행의 정상적인 조기 `partial` 종료를 방해하지 않게 했다.
- 테스트가 바뀌는 9.5 적합성 보정 라운드의 회귀 순서를 6단계 테스트 → 8단계 커버리지 → 9단계 적합성으로 고정했다.
- MCP 패키지 버전을 `0.11.1`로 올렸다.
