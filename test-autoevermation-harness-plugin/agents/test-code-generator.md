...
- `/test-autoevermation-harness-plugin:generate-tests` skill이 직접 호출될 때
...
- `/test-autoevermation-harness-plugin:generate-tests` — 이 에이전트를 단독 호출하는 skill
- `/test-autoevermation-harness-plugin:full-pipeline` — 시나리오 확정 후 이 에이전트를 호출
