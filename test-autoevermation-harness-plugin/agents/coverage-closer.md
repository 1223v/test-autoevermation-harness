...
- `/test-autoevermation-harness-plugin:measure-coverage` 스킬이 coverage_gate 결과가 `failed` 또는 `partial`일 때 호출
...
- `/test-autoevermation-harness-plugin:measure-coverage` — 이 에이전트를 커버리지 갭 감지 시 호출
- `/test-autoevermation-harness-plugin:full-pipeline` — 커버리지 gap-closing 루프에서 호출
