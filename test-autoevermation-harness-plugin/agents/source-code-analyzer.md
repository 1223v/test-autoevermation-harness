...
- **역할**: "정의로 이동(go-to-definition)", "참조 찾기(find-references)"로 다형성 호출·인터페이스 바인딩의 semantic 보강. **이 에이전트의 tools에 LSP 도구는 없다** — JDT LS는 Phase E·E7이 세션 레벨에서 보장하는 전제이며(`lspAvailable=true`), 에이전트 자신의 추적 수단은 repo-ast(`resolve_symbol`/`parse_java_file`)다.
