...
이 에이전트는 **읽기 전용**이다. 구조가 아닌 동작을 분석하는 것이 핵심 차이점이다. **JDT LS는 필수**다([fallback-policy.md](../references/fallback-policy.md) #3): 정의 이동·참조 탐색에 사용하며, **미가용 시 중단**한다(`status: "failed"` + `errors: JDT_LS_UNAVAILABLE` + remediation: `node <pluginRoot>/mcp/launch.cjs script <pluginRoot>/scripts/setup_jdtls.py` 실행, Java 21+ runtime 필요).
...
- **가용 조건**: JDT LS가 연결되어 있고 Java 21+ runtime이 가용한 경우
- **미가용 시**: 중단한다(fallback-policy.md #3). `status: "failed"` + `errors`에 `JDT_LS_UNAVAILABLE` 기록 + remediation(`node <pluginRoot>/mcp/launch.cjs script <pluginRoot>/scripts/setup_jdtls.py` 실행, Java 21+ 필요)
...
| 실패 클래스 | 조건 | 대응 |
|---|---|---|
| `JDT_LS_UNAVAILABLE` | `lspAvailable: false` 또는 JDT LS 연결 실패 | `status:"failed"` 반환. `errors`에 `JDT_LS_UNAVAILABLE` 기록. remediation: `node <pluginRoot>/mcp/launch.cjs script <pluginRoot>/scripts/setup_jdtls.py` 실행(Java 21+ 필요) |
...
JDT LS 미가용은 **중단 조건**이다(fallback-policy.md #3): `status:"failed"` + `errors`에 `JDT_LS_UNAVAILABLE` + remediation(`node <pluginRoot>/mcp/launch.cjs script <pluginRoot>/scripts/setup_jdtls.py`, Java 21+ 필요). AST-only degrade는 더 이상 허용되지 않는다.
