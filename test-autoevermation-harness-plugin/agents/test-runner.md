...
- `/test-autoevermation-harness-plugin:run-tests` skill이 직접 호출될 때
...
- `/test-autoevermation-harness-plugin:run-tests` — 이 에이전트를 단독 호출하는 skill
- `/test-autoevermation-harness-plugin:full-pipeline` — 테스트 파일 생성 후 이 에이전트를 호출하고, 실패 시 `test-fixer`로 전달
