...
배포 대상 플러그인(`spring-test-harness-plugin/`)을 포함한다.
...
플러그인 버전은 `spring-test-harness-plugin/.claude-plugin/plugin.json`의 `version`에 고정되어,
...
동작 원리·사용법 종합 가이드는 [`spring-test-harness-plugin/docs/GUIDE.md`](spring-test-harness-plugin/docs/GUIDE.md)를 참조한다.
