...
이 문서는 `spring-test-harness` 플러그인이 **어떤 구조로, 어떤 순서로 동작하는지**와
...
명령 하나(`/spring-test-harness:full-pipeline`)로 아래 전 과정을 오케스트레이션한다.
...
- **독립 실행**: oh-my-claudecode 등 외부 플러그인에 의존하지 않는다. Claude Code 네이티브 기능
  (플러그인·`Task` 서브에이전트·`AskUserQuestion`·MCP·훅)과 Python 3.10+만 필수다. 상세: [DEPENDENCIES.md](../DEPENDENCIES.md).
...
├── skills/        (13종)        ← /spring-test-harness:<name> 명령. 절차(무엇을 어떤 순서로)의 정의
...
모든 스킬은 단독 호출도 가능하다(`/spring-test-harness:<skill>`). `full-pipeline`은 이들을 순서·병렬
전략에 따라 조합한다.
...
사용자에게 전달됨). 수동 설치를 원하면 `spring-test-harness-plugin/`을 `~/.claude/plugins/` 아래에
복사(또는 symlink)하고 Claude Code를 재시작해도 된다.

설치 확인: 세션에서 `/spring-test-harness:full-pipeline` 명령이 자동완성에 뜨면 정상.

### 4.2 MCP 런타임 (필수)

MCP 서버 3종은 Python으로 돈다. **`.mcp.json`이 실행하는 바로 그 `python3`**에 설치해야 한다
(`which python3`로 확인):
...
```text
/spring-test-harness:analyze-ast          # 테스트 대상·구조만 추출
/spring-test-harness:refactor-advisory    # 테스트 부적합 코드 판정만 (read-only)
/spring-test-harness:measure-coverage     # 기존 테스트로 커버리지 게이트만
/spring-test-harness:verify-scenarios     # 시나리오 충족 여부만 재검증
```
...
```bash
claude -p --output-format json "/spring-test-harness:full-pipeline {
  \"projectRoot\": \"$PWD\",
...
configure-harness 마지막의 **도메인 스킬 스캐폴딩**으로 `/spring-test-harness:<custom>` 스킬을 만들어 둘 수 있다.
...
| 증상 | 원인 | 해결 |
|---|---|---|
| MCP 서버 3종이 연결 안 됨 | `mcp` 패키지가 다른 python3에 설치됨 | `which python3` 확인 후 그 인터프리터에 `python3 -m pip install -r mcp/requirements.txt` |
