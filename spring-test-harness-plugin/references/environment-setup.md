...
|---|---|---|---|---|---|---|
| E1 | **Python 3.10+** | `python3 -c "import sys;assert sys.version_info>=(3,10)"` | assist | 미충족 시 AskUserQuestion으로 설치 경로 안내(pyenv/brew). 없으면 중단 | 하드 중단 + remediation | MCP 런타임 |
| E2 | **MCP Python SDK** (`mcp[cli]>=1.2.0`) | `python3 -c "import mcp"` | auto | `AskUserQuestion("MCP SDK 설치할까요?")` → 예: `python3 -m pip install -r mcp/requirements.txt` | **자동** `python3 -m pip install -r mcp/requirements.txt` | policy #1 |
...
```bash
# E2: MCP Python SDK (Python 3.10+)
python3 -m pip install -r mcp/requirements.txt          # → mcp[cli]>=1.2.0
#   주의: .mcp.json이 실행하는 동일 python3에 설치해야 함 (which python3 로 확인)
