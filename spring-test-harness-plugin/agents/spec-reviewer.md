...
- `/spring-test-harness:ingest-specs` skill이 직접 호출될 때
...
- `/spring-test-harness:ingest-specs` — 이 에이전트를 단독 호출하는 skill
- `/spring-test-harness:full-pipeline` — 파이프라인 1단계에서 AST 분석과 병렬로 이 에이전트를 호출
