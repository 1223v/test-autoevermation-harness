...
이 에이전트는 **읽기 전용**이다. 구조가 아닌 동작을 분석하는 것이 핵심 차이점이다. JDT LS(선택)가 가용하면 정의 이동·참조 탐색에 활용하고, 미가용 시 AST-only로 부분 분석(partial)을 반환한다.
...
- **미가용 시**: AST-only로 분석하고 `status: partial`, `warnings`에 "JDT LS 미가용으로 다형성 호출 추적 제한됨" 기록
...
각 대상의 외부 의존(DB/HTTP/clock/random)을 식별해 mocking seam을 제안하라. 동작 흐름과 예외 경로를 분리해 기술하라. 협력 객체는 Spring 빈 Mock 애노테이션(프로파일에 따라 `@MockBean`/`@MockitoBean`)으로 대체 가능한지 `mockable` 여부를 표시하라. JDT LS가 가용하면 인터페이스 바인딩을 정밀 추적하고, 미가용이면 AST-only로 partial 반환한다.
...
JDT LS 미가용은 실패가 아니라 `partial`이다. `errors`가 아닌 `warnings`에 기록한다.
