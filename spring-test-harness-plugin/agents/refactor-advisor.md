...
- `/spring-test-harness:refactor-advisory` skill이 직접 호출될 때
...
- `/spring-test-harness:refactor-advisory` — 이 에이전트를 단독 호출하는 skill
- `/spring-test-harness:full-pipeline` — 3.5단계에서 호출(결과로 권고 `.md` 저장 + 결정 게이트 수행)
